...
#### Obtain data, assign it to a variable and view it
...
3) Use `tail()` to visualize the end of the pancreas `data.frame` 
```{r}
...
9) Use the function `head()` to view the first few lines of pancreas. 
```{r}
class(pancreas)
...
12) View the first few lines of sra.
```{r}
...
24) Visualize beta_cell_samples by typing its name. 
```{r}
...
26) Visualize adult_cell_samples. Besides typing the name of the variable containing the data you wish to visualize, you can visualize the results of a command you just typed by placing the entire command in parentheses. For this answer, copy your command and add parentheses around it.
```{r}
...
31) Visualize adult_or_beta_cell_samples.
...
43) We are interested in only those genes for which both samples SRR1951541 or SRR1951542 have gene expression values greater than zero. First, report TRUE/FALSE for genes (rows) where this is the case using the `|` operator. Assign it to the name "one_greaterthanzero".
44) Subset the pancreas `data.frame` where both SRR1951541 and SRR1951542 have gene expression values greater than zero. Assign it to the name "both_greaterthanzero_all_data".
45) Build a boxplot from the log2(x+1) of the expression values across all samples where SRR1951541 and SRR1951542 have a gene expression value greater than zero.
```{r}
...
48) Visualize sra_adult_beta to be sure it outputs what you expect.
...
50) Visualize coi to be sure it outputs what you expect. 
...
coi
boxplot(log2(rowMeans(pancreas[,coi],na.rm=FALSE)+1))
```
