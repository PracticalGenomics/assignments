...
In the same way as above, assign the numbers of instances a column has greater than zero values to the name "pan_greaterthanzero".
...
Finally, we can make a scatterplot of these means using the function `plot()`. Using `main=` we can name the plot, and we are also labeling the datapoints with the row number in order to quickly find gene names that show an obvious difference in expression.
